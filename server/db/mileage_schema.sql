-- Mileage claim schema for Facilitate Care Services.
...
CREATE TABLE IF NOT EXISTS mileage_entries (
  ...
  CONSTRAINT chk_mileage_odometer CHECK (odometer_end >= odometer_start),
  CONSTRAINT chk_mileage_non_negative CHECK (
    odometer_start >= 0 AND odometer_end >= 0 AND claimed_mileage >= 0
    AND expected_system_mileage >= 0 AND passenger_pickup_mileage >= 0 AND midday_payable_mileage >= 0 AND lunch_home_mileage_deduction >= 0
    AND adjusted_claimed_mileage >= 0 AND mileage_rate >= 0
  )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Run this ALTER on existing installations that created mileage_entries before passenger pickup support.
ALTER TABLE mileage_entries
  ADD COLUMN IF NOT EXISTS passenger_pickup_mileage DECIMAL(10,2) NOT NULL DEFAULT 0.00 AFTER expected_system_mileage;

-- Run these ALTERs on existing installations that created mileage_entries before authorised midday additions.
ALTER TABLE mileage_entries
  ADD COLUMN IF NOT EXISTS midday_payable_mileage DECIMAL(10,2) NOT NULL DEFAULT 0.00 AFTER passenger_pickup_mileage;

ALTER TABLE mileage_entries
  ADD COLUMN IF NOT EXISTS midday_mileage_reason VARCHAR(255) NULL AFTER midday_payable_mileage;
...
CREATE TABLE IF NOT EXISTS mileage_expected_routes (
  ...
  CONSTRAINT chk_expected_route_non_negative CHECK (expected_mileage >= 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Permission seed for the existing Facilitate role_permissions table.
-- The frontend also has this permission in src/utils/accessControl.js.
