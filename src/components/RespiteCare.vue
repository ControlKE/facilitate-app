<template>
	...
	<!--Page Title-->
    <section class="page-title" :style="{ backgroundImage: `url(${cmsValue('respitecare', 'hero', 'background_image_url', '/frontend/images/background/3.jpg')})` }">
    	<div class="auto-container">
        	<h2>{{ cmsValue('respitecare', 'hero', 'headline', 'Respite Care') }}</h2>
            <ul class="page-breadcrumb">
            	<li><a href="/">home</a></li>
                <li>{{ cmsValue('respitecare', 'hero', 'headline', 'Respite Care') }}</li>
            </ul>
        </div>
    </section>
    <!--End Page Title-->
        	...
        	<div class="row clearfix">
                ...
                <div class="content-side col-lg-8 col-md-12 col-sm-12">
                	<div class="services-detail">
						<div class="inner-box">
							<div class="image">
								<img :src="cmsValue('respitecare', 'content', 'primary_image_url', '/frontend/images/resource/service-19.jpg')" alt="" />
							</div>
							<div class="lower-content">
								<h2>{{ cmsValue('respitecare', 'content', 'headline', 'Respite Care Service') }}</h2>
								<p>{{ cmsValue('respitecare', 'content', 'overview_text_1', 'Respite care is a temporary arrangement designed to provide relief for primary caregivers. Whether you need support for a short-term basis due to your regular caregiver being unavailable, or you require assistance following surgery or hospitalization, respite care offers the perfect solution. This service ensures that your loved ones continue to receive the highest quality care while allowing primary caregivers to take a much-needed break.') }}</p>
								<p>{{ cmsValue('respitecare', 'content', 'overview_text_2', 'We understand the challenges of caregiving and are committed to providing the break you need to rejuvenate, while ensuring your loved ones continue to live comfortably and independently in their own homes.') }}</p>
								<!-- <p>Let our experienced caregivers help your family</p> -->
								<div class="two-column">
									<div class="row clearfix">
										<div class="column col-lg-4 col-md-4 col-sm-12">
											<div class="image">
												<img :src="cmsValue('respitecare', 'content', 'secondary_image_url', '/frontend/images/resource/service-20.jpg')" alt="" />
											</div>
										</div>
										<div class="column col-lg-8 col-md-8 col-sm-12">
											<h4>{{ cmsValue('respitecare', 'content', 'secondary_headline', 'Tailored Respite Care Services') }}</h4>
											<p>{{ cmsValue('respitecare', 'content', 'secondary_text', 'Our respite care services are designed to give you, as a caregiver, a break from your daily duties so you can recharge. Recognizing your vital role, we emphasize self-care while ensuring your loved one receives the best possible attention at home. Our tailored care plans meet each individual\'s unique needs, with compassionate, skilled caregivers dedicated to delivering exceptional support and providing you with assurance and peace of mind.') }}</p>
										</div>
									</div>
								</div>
								<h3>{{ cmsValue('respitecare', 'content', 'services_headline', 'Personalized Respite Care Services') }}</h3>
								<p>{{ cmsValue('respitecare', 'content', 'services_intro_text', 'At Facilitate Care Services, we understand the importance of continuity and familiarity in care. Our respite care is designed to fit seamlessly into your existing routine, ensuring that you or your loved one feels comfortable and secure. Here\'s what you can expect from our comprehensive respite care services:') }}</p>
								<ul class="list-style-three">
									<li>{{ cmsValue('respitecare', 'content', 'service_item_1', 'Personalized Support: We tailor our care plans to meet the unique needs and preferences of each individual. This includes assistance with personal grooming, bathing, and toileting.') }}</li>
									<li>{{ cmsValue('respitecare', 'content', 'service_item_2', 'Mobility and Transfers: Our caregivers are trained to help with mobility, ensuring safe transfers from bed to chair, and assisting with walking.') }}</li>
									<li>{{ cmsValue('respitecare', 'content', 'service_item_3', 'Medication Management: We provide reliable medication reminders and management to ensure health and well-being.') }}</li>
									<li>{{ cmsValue('respitecare', 'content', 'service_item_4', 'Companionship: Emotional support and companionship are key components of our care, helping to alleviate loneliness and provide social interaction.') }}</li>
									<li>{{ cmsValue('respitecare', 'content', 'service_item_5', 'Housekeeping and Meal Preparation: Our caregivers can assist with light housekeeping tasks and prepare nutritious meals to suit dietary requirements.') }}</li>
									<li>{{ cmsValue('respitecare', 'content', 'service_item_6', 'Errands and Shopping: We can run errands and handle shopping needs, bringing convenience to your doorstep.') }}</li>
									<li>{{ cmsValue('respitecare', 'content', 'service_item_7', 'Bedtime Routines: Assistance with evening routines ensures a comfortable and restful night.') }}</li>
								</ul>
								<h3>{{ cmsValue('respitecare', 'content', 'trust_headline', 'Respite Care you can trust') }}</h3>
								<p>{{ cmsValue('respitecare', 'content', 'trust_text', 'Our team commitment to providing compassionate, skilled, and personalized care makes a significant difference in the lives of those we serve. By choosing our respite care service, you are ensuring that your loved one is in capable and caring hands. Our flexible care plans adapt to your loved one needs, offering peace of mind for you and quality care for them. Trust us to provide the support needed to maintain their well-being and happiness.') }}</p>
								<div class="service-contact-box"> Let our experienced caregivers help your family today. <br> <router-link to="/contact">Contact our team</router-link> to learn more! </div>
							</div>
						</div>
					</div>
				</div>
              ...
              <ul class="services-list">
				                <li><router-link to="/personalcare">Personal Care</router-link></li>
								<li><router-link to="/elderlyservice">Elderly Care Service</router-link></li>
								<li class="active"><router-link to="/respitecare">Respite Care</router-link></li>
								<li><router-link to="/livein">Live In Care</router-link></li>
                        ...
                        <div class="sidebar-widget contact-widget">
							<div class="widget-content" style="background-image: url(/frontend/images/resource/contact-2.jpg);">
								<h3>{{ cmsValue('respitecare', 'sidebar', 'headline', 'Find Care Today') }}</h3>
								<router-link to="/contact" class="theme-btn contact-btn">{{ cmsValue('respitecare', 'sidebar', 'button_text', 'contact us') }}</router-link>
							</div>
						</div>
