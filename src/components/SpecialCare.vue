<template>
	...
	<!--Page Title-->
    <section class="page-title" :style="{ backgroundImage: `url(${cmsValue('specialcare', 'hero', 'background_image_url', '/frontend/images/background/3.jpg')})` }">
    	<div class="auto-container">
        	<h2>{{ cmsValue('specialcare', 'hero', 'headline', 'Special Needs Care') }}</h2>
            <ul class="page-breadcrumb">
            	<li><a href="/">home</a></li>
                <li>{{ cmsValue('specialcare', 'hero', 'headline', 'Special Needs Care') }}</li>
            </ul>
        </div>
    </section>
    <!--End Page Title-->
        	...
        	<div class="row clearfix">
                ...
                <div class="content-side col-lg-8 col-md-12 col-sm-12">
                	<div class="services-detail">
						<div class="inner-box">
							<div class="image">
								<img :src="cmsValue('specialcare', 'content', 'primary_image_url', '/frontend/images/resource/service-26.jpg')" alt="" />
							</div>
							<div class="lower-content">
								<h2>{{ cmsValue('specialcare', 'content', 'headline', 'Special Needs Care') }}</h2>
								<p>{{ cmsValue('specialcare', 'content', 'overview_text_1', 'Special Needs Care encompasses services designed to support individuals with disabilities, focusing on improving their quality of life and maximizing their potential. This includes assistance with daily activities, medical management, and therapeutic interventions like physical and occupational therapy. It also offers educational support, behavioral strategies, respite care for caregivers, and emotional and social support to foster community integration. Utilizing adaptive equipment and technology, special needs care involves a multidisciplinary team to address the holistic needs of individuals.') }}</p>
								<p>{{ cmsValue('specialcare', 'content', 'overview_text_2', 'At Facilitate Care Services, we are dedicated to providing exceptional special needs care, ensuring the highest quality of life for your loved one. Our highly trained and certified caregivers deliver personalized care tailored to each individual\'s unique requirements. Our approach includes comprehensive daily assistance, medical management, and specialized therapies to meet your loved one\'s holistic needs. We also implement positive behavioral strategies and encourage community involvement to promote social and emotional well-being.') }}</p>
								
								<div class="two-column">
									<div class="row clearfix">
										<div class="column col-lg-4 col-md-4 col-sm-12">
											<div class="image">
												<img :src="cmsValue('specialcare', 'content', 'secondary_image_url', '/frontend/images/resource/service-25.jpg')" alt="" />
											</div>
										</div>
										<div class="column col-lg-8 col-md-8 col-sm-12">
											<h4>{{ cmsValue('specialcare', 'content', 'secondary_headline', 'Special Needs Care Services to our Clients') }}</h4>
											<p>{{ cmsValue('specialcare', 'content', 'secondary_text', 'Many seniors prefer to age in their own home, but this can grow more and more difficult as the years pass. Your loved one doesn\'t need to give up on the familiarity and comfort of their home, simply because they cannot remain completely independent. Our personal care services are specially designed to provide additional assistance to enhance your loved one\'s independence.') }}</p>
										</div>
									</div>
								</div>
								<h3>{{ cmsValue('specialcare', 'content', 'services_headline', 'Our personal caregivers offer assistance with activities such as:') }}</h3>
								<ul class="list-style-three">
									<li>{{ cmsValue('specialcare', 'content', 'services_item_1', 'Personal grooming, bathing, toileting, and hygiene tasks') }}</li>
									<li>{{ cmsValue('specialcare', 'content', 'services_item_2', 'Mobility and transfers') }}</li>
									<li>{{ cmsValue('specialcare', 'content', 'services_item_3', 'Medication reminders and monitoring') }}</li>
									<li>{{ cmsValue('specialcare', 'content', 'services_item_4', 'Companionship') }}</li>
									<li>{{ cmsValue('specialcare', 'content', 'services_item_5', 'Light housekeeping, meal planning, and meal preparation') }}</li>
									<li>{{ cmsValue('specialcare', 'content', 'services_item_6', 'Shopping and errands') }}</li>
								</ul>
								<h3>{{ cmsValue('specialcare', 'content', 'all_needs_headline', 'Special Needs Care for All Needs') }}</h3>
								<p>{{ cmsValue('specialcare', 'content', 'closing_text', 'Our caregivers are ready to provide quality services to everyone in need of assistance, including elders, those who are living with a disability, and those who are in recovery from an injury or living with a chronic condition. We focus on your individual needs, regardless of the reason you need some extra help. Let us help your day-to-day life be simple, comfortable, and safe.') }}</p>
								<div class="service-contact-box">
                  {{ cmsValue('specialcare', 'content', 'service_cta_text', 'Let our experienced caregivers help your family today.') }}
                  <br>
                  <router-link to="/contact">{{ cmsValue('specialcare', 'content', 'service_cta_link_text', 'Contact our team') }}</router-link>
                  {{ cmsValue('specialcare', 'content', 'service_cta_suffix', 'to learn more!') }}
                </div>
							</div>
						</div>
					</div>
					...
					<aside class="sidebar padding-left">
                        ...
                        <div class="sidebar-widget list-widget">
							...
							<ul class="services-list">
								...
								<li class="active"><router-link to="/specialcare">Special Needs Care</router-link></li>
							</ul>
						</div>
                        ...
                        <div class="sidebar-widget contact-widget">
							<div class="widget-content" style="background-image: url(/frontend/images/resource/contact-2.jpg);">
								<h3>{{ cmsValue('specialcare', 'sidebar', 'headline', 'Find Care Today') }}</h3>
								<router-link to="/contact" class="theme-btn contact-btn">{{ cmsValue('specialcare', 'sidebar', 'button_text', 'contact us') }}</router-link>
							</div>
						</div>
