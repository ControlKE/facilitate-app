<template>
	...
	<!--Page Title-->
    <section class="page-title" :style="{ backgroundImage: `url(${cmsValue('chronical', 'hero', 'background_image_url', '/frontend/images/background/3.jpg')})` }">
    	<div class="auto-container">
        	<h2>{{ cmsValue('chronical', 'hero', 'headline', 'Palliative Care') }}</h2>
             <ul class="page-breadcrumb">
             	<li><a href="/">home</a></li>
                <li>{{ cmsValue('chronical', 'hero', 'headline', 'Cronical Condition Care') }}</li>
            </ul>
        </div>
    </section>
    <!--End Page Title-->
    	...
    	<div class="auto-container">
        	<div class="row clearfix">
                	...
                	<div class="services-detail">
						<div class="inner-box">
							<div class="lower-content">
								<h2>{{ cmsValue('chronical', 'content', 'headline', 'Palliative Care') }}</h2>
								<div class="styled-text">{{ cmsValue('chronical', 'content', 'subheadline', 'Personalized Care for Your Daily Needs') }}</div>
								<p>{{ cmsValue('chronical', 'content', 'overview_text', 'Palliative care is a specialized medical approach aimed at improving the quality of life for patients with serious, life-threatening illnesses. It focuses on providing relief from the symptoms, pain, and stress associated with these conditions. The primary goal is to enhance the quality of life for both the patient and their family, addressing not just physical symptoms but also psychological, social and spiritual issues.') }}</p>
								<p>{{ cmsValue('chronical', 'content', 'intro_text', 'With our experienced and caring staff, your loved one can remain in their home and continue living independently for longer.') }}</p>
								<div class="styled-text service-contact-box">{{ cmsValue('chronical', 'content', 'testimonial_quote', '" Facilitate Care looked after and cared for my husband Don in the last weeks of his life . They were amazing very gentle spoke to Don as they were caring for him I was very happy with the care they gave him . Ekta and Eric were very special people and I thank them with all my heart . "  Theresa B.') }}</div>
								<div class="two-column">
									<div class="row clearfix">
										<div class="column col-lg-4 col-md-4 col-sm-12">
											<div class="image">
												<img :src="cmsValue('chronical', 'content', 'primary_image_url', '/frontend/images/resource/service-14.jpg')" alt="" />
											</div>
										</div>
										<div class="column col-lg-8 col-md-8 col-sm-12">
											<h4>{{ cmsValue('chronical', 'content', 'secondary_headline', 'Comprehensive Palliative Care Services') }}</h4>
											<p>{{ cmsValue('chronical', 'content', 'secondary_text', 'Palliative care with us begins with a comprehensive conversation to understand the patient symptoms and needs. Our caregivers follow recommendations from healthcare professionals to tailor treatments to individual comfort levels. We regularly communicate with patients to assess their pain and other symptoms, collaborating with professionals to determine the best treatment options. If a patient is in pain or uncomfortable, we promptly report to the relevant professionals for assessment, ensuring personalized and compassionate care that enhances the patient quality of life.') }}</p>
										</div>
									</div>
								</div>
								<h3>{{ cmsValue('chronical', 'content', 'goals_headline', 'Goals of Palliative Care') }}</h3>
											<ul class="list-style-three">
												<li>{{ cmsValue('chronical', 'content', 'goals_item_1', 'Aligning treatment outcomes with the patient values and preferences') }}</li>
												<li>{{ cmsValue('chronical', 'content', 'goals_item_2', 'Improving quality of life for both the patient and the family') }}</li>
												<li>{{ cmsValue('chronical', 'content', 'goals_item_3', 'Minimizing pain and discomfort') }}</li>
												<li>{{ cmsValue('chronical', 'content', 'goals_item_4', 'Alleviating emotional distress, anxiety, or depression') }}</li>
												<li>{{ cmsValue('chronical', 'content', 'goals_item_5', 'Assisting with safety, mobility, and equipment') }}</li>
												<li>{{ cmsValue('chronical', 'content', 'goals_item_6', 'Empowering patients and caregivers to make decisions that are right for them') }}</li>
											</ul>
								<h3>{{ cmsValue('chronical', 'content', 'getting_started_headline', 'Getting Started') }}</h3>
								<p>{{ cmsValue('chronical', 'content', 'getting_started_text_1', 'To get started with palliative care at Facilitate Care Services, the first step is to schedule an initial consultation with our palliative care team. During this meeting, we will discuss your symptoms, medical history, and care goals, allowing us to create a personalized care plan tailored to your specific needs and comfort levels.') }}</p>
								<p>{{ cmsValue('chronical', 'content', 'getting_started_text_2', 'Once the initial assessment is complete, our team will collaborate with you, your family, and other healthcare providers to develop a comprehensive care plan. This plan will focus on managing pain, alleviating symptoms, and providing emotional support to improve your quality of life. Regular reviews and adjustments of the care plan ensure that it continues to meet your evolving needs and preferences.') }}</p>
								<p>{{ cmsValue('chronical', 'content', 'getting_started_text_3', 'Palliative care at Facilitate Care Services is an ongoing process. We will continuously support you by coordinating with other medical professionals and offering guidance and assistance to you and your family. Our goal is to ensure that you receive the best possible care and maintain a high quality of life throughout your journey.') }}</p>
								<div class="service-contact-box">
                  {{ cmsValue('chronical', 'content', 'getting_started_cta_text', 'Starting palliative care with us involves open communication, thorough planning, and continuous support.') }}
                  <router-link to="/contact">{{ cmsValue('chronical', 'content', 'getting_started_cta_link_text', 'Contact our team') }}</router-link>
                  {{ cmsValue('chronical', 'content', 'getting_started_cta_suffix', 'to learn more!') }}
                </div>
							</div>
              ...
              <ul class="services-list">
								...
								<li><router-link to="/discharge">Hospital Discharge</router-link></li>
								<li><router-link to="/care">Social Care</router-link></li>
								<li class="active"><router-link to="/chronical">Palliative Care</router-link></li>
								<li><router-link to="/support">Supported Living</router-link></li>
                        ...
                        <div class="sidebar-widget contact-widget">
							<div class="widget-content" style="background-image: url(/frontend/images/resource/contact-2.jpg);">
								<h3>{{ cmsValue('chronical', 'sidebar', 'headline', 'Find Care Today') }}</h3>
								<router-link to="/contact" class="theme-btn contact-btn">{{ cmsValue('chronical', 'sidebar', 'button_text', 'contact us') }}</router-link>
							</div>
						</div>
