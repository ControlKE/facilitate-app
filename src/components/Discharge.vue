<template>
	...
	<!--Page Title-->
    <section class="page-title" :style="{ backgroundImage: `url(${cmsValue('discharge', 'hero', 'background_image_url', '/frontend/images/background/3.jpg')})` }">
    	<div class="auto-container">
        	<h2>{{ cmsValue('discharge', 'hero', 'headline', 'Hospital Discharge') }}</h2>
            <ul class="page-breadcrumb">
            	<li><a href="/">home</a></li>
                <li>{{ cmsValue('discharge', 'hero', 'headline', 'Hospital Discharge') }}</li>
            </ul>
        </div>
    </section>
    <!--End Page Title-->
                	...
                	<div class="services-detail">
						<div class="inner-box">
							<div class="lower-content">
								<h2>{{ cmsValue('discharge', 'content', 'headline', 'Supporting your return from hospital.') }}</h2>
								<div class="styled-text">{{ cmsValue('discharge', 'content', 'subheadline', 'Home care after hospital discharge.') }}</div>
								<p>{{ cmsValue('discharge', 'content', 'intro_text', 'We recognize that the transition from hospital to home can be overwhelming.') }}</p>
								<p>{{ cmsValue('discharge', 'content', 'overview_text', 'Facilitate Care Services excels in providing the essential support needed for a smooth recovery at home. Whether it\'s short-term care following a hospital discharge, rehabilitation care to aid in recovery and the relearning of daily tasks, or long-term solutions such as visiting or live-in care, we customize our services to meet your specific needs.') }}</p>
								<div class="styled-text service-contact-box">{{ cmsValue('discharge', 'content', 'testimonial_quote', '" After a nasty fall and a week stay in hospital, I was nervous about having a care package at home, especially as a nurse and only just being 60. I had no need the 2 ladies who come are wonderful, cope with my wobbles and I feel so cared for." Debbie S.') }}</div>
								<p>{{ cmsValue('discharge', 'content', 'support_text', 'If you or a loved one is approaching a hospital discharge and requires post-hospital care, whether for a brief period or on an ongoing basis, Facilitate Care Services is here to offer the assistance and support necessary for a successful recovery.') }}</p>
								<div class="two-column">
									<div class="row clearfix">
										<div class="column col-lg-4 col-md-4 col-sm-12">
											<div class="image">
												<img :src="cmsValue('discharge', 'content', 'primary_image_url', '/frontend/images/resource/service-22.jpg')" alt="" />
											</div>
										</div>
										<div class="column col-lg-8 col-md-8 col-sm-12">
											<h4>{{ cmsValue('discharge', 'content', 'secondary_headline', 'Comprehensive Discharge Planning') }}</h4>
											<p>{{ cmsValue('discharge', 'content', 'secondary_text', 'Our dedicated team works closely with hospital staff, patients, and families to develop a personalized discharge plan that addresses medical and emotional needs. We ensure that every aspect of the transition is carefully coordinated, minimizing stress and maximizing comfort.') }}</p>
										</div>
									</div>
								</div>
								<h3>{{ cmsValue('discharge', 'content', 'care_coordination_headline', 'Personalized Care Coordination') }}</h3>
								<p>{{ cmsValue('discharge', 'content', 'care_coordination_text', 'Upon discharge, our professional caregivers provide personalized assistance to meet the unique needs of each patient. Services include:') }}</p>
								<ul class="list-style-three">
									<li>{{ cmsValue('discharge', 'content', 'care_coordination_item_1', 'Medication Management: Ensuring that prescriptions are filled and administered correctly.') }}</li>
									<li>{{ cmsValue('discharge', 'content', 'care_coordination_item_2', 'Personal Care Assistance: Helping with daily activities such as bathing, dressing, and grooming.') }}</li>
									<li>{{ cmsValue('discharge', 'content', 'care_coordination_item_3', 'Nutritional Support: Preparing meals that meet dietary requirements and support recovery.') }}</li>
									<li>{{ cmsValue('discharge', 'content', 'care_coordination_item_4', 'Emotional Support: Offering companionship and emotional encouragement to promote overall well-being.') }}</li>
								</ul>
								<h3>{{ cmsValue('discharge', 'content', 'monitoring_headline', 'Follow-Up and Monitoring') }}</h3>
								<p>{{ cmsValue('discharge', 'content', 'monitoring_text', 'We provide ongoing follow-up and monitoring to ensure that the patient is progressing well and adhering to their recovery plan. Our caregivers are trained to recognize signs of complications and communicate with healthcare providers to address any concerns promptly.') }}</p>
								<h3>{{ cmsValue('discharge', 'content', 'benefits_headline', 'Benefits of Our Hospital Discharge Service') }}</h3>
								<ul class="list-style-three">
									<li>{{ cmsValue('discharge', 'content', 'benefits_item_1', 'Enhanced Recovery: Professional care that promotes quicker and more effective recovery.') }}</li>
									<li>{{ cmsValue('discharge', 'content', 'benefits_item_2', 'Peace of Mind: Knowing that your loved one is receiving expert care and support.') }}</li>
									<li>{{ cmsValue('discharge', 'content', 'benefits_item_3', 'Reduced Re-admission Rates: Proper post-hospital care reduces the likelihood of re-admission.') }}</li>
									<li>{{ cmsValue('discharge', 'content', 'benefits_item_4', 'Convenience: Comprehensive service that handles all aspects of the discharge process.') }}</li>
								</ul>
								<p>{{ cmsValue('discharge', 'content', 'closing_text', 'At Facilitate Care Services, our goal is to provide a seamless and compassionate transition from hospital to home, ensuring that our clients receive the highest quality care during their recovery journey. Trust us to be your partner in health and well-being.') }}</p>
								<div class="service-contact-box">
                  {{ cmsValue('discharge', 'content', 'service_cta_text', 'Let our experienced caregivers help your family today.') }}
                  <br>
                  <router-link to="/contact">{{ cmsValue('discharge', 'content', 'service_cta_link_text', 'Contact our team') }}</router-link>
                  {{ cmsValue('discharge', 'content', 'service_cta_suffix', 'to learn more!') }}
                </div>
							</div>
						</div>
					</div>
					...
					<aside class="sidebar padding-left">
                        ...
                        <div class="sidebar-widget list-widget">
							...
							<ul class="services-list">
								...
								<li><router-link to="/respitecare">Respite Care</router-link></li>
								<li><router-link to="/livein">Live In Care</router-link></li>
								<!--li><a href="surgery.html">24/7 Day Support</a></li-->
								<li class="active"><router-link to="/discharge">Hospital Discharge</router-link></li>
								<li><router-link to="/care">Social Care</router-link></li>
								<li><router-link to="/chronical">Palliative Care</router-link></li>
								<li><router-link to="/support">Supported Living</router-link></li>
								<!--li><a href="support.html">After Surgery Care</a></li>
                        ...
                        <div class="sidebar-widget contact-widget">
							<div class="widget-content" style="background-image: url(/frontend/images/resource/contact-2.jpg);">
								<h3>{{ cmsValue('discharge', 'sidebar', 'headline', 'Find Care Today') }}</h3>
								<router-link to="/contact" class="theme-btn contact-btn">{{ cmsValue('discharge', 'sidebar', 'button_text', 'contact us') }}</router-link>
							</div>
						</div>
