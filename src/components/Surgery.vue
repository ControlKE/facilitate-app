<template>
	...
	<!--Page Title-->
    <section class="page-title" :style="{ backgroundImage: `url(${cmsValue('surgery', 'hero', 'background_image_url', '/frontend/images/background/3.jpg')})` }">
    	<div class="auto-container">
        	<h2>{{ cmsValue('surgery', 'hero', 'headline', '24/7 Day Support') }}</h2>
            <ul class="page-breadcrumb">
            	<li><a href="/">home</a></li>
                <li>{{ cmsValue('surgery', 'hero', 'headline', '24/7 Day Support') }}</li>
            </ul>
        </div>
    </section>
    <!--End Page Title-->
        	...
        	<div class="row clearfix">
                ...
                <div class="content-side col-lg-8 col-md-12 col-sm-12">
                	<div class="services-detail">
						<div class="inner-box">
							<div class="image">
								<img :src="cmsValue('surgery', 'content', 'primary_image_url', '/frontend/images/resource/service-8.jpg')" alt="" />
							</div>
							<div class="lower-content">
								<h2>{{ cmsValue('surgery', 'content', 'headline', 'We Give 24/7 Day Support') }}</h2>
								<p>{{ cmsValue('surgery', 'content', 'overview_text_1', 'Do you or a loved one need a little bit of extra help in your home? Chores, errands, and personal tasks can be difficult or unsafe for many individuals to perform on their own. When your loved one requires assistance with bathing and dressing, our personal care services are designed just for their needs.') }}</p>
								<p>{{ cmsValue('surgery', 'content', 'overview_text_2', 'With our experienced and caring staff, your loved one can remain in their home and continue living independently for longer. Our customized care plans will match your loved one needs exactly, so they can enjoy care services that are uniquely tailored to them. Our compassionate caregivers are here to provide your loved one with exceptional support, while giving you the peace of mind you deserve.') }}</p>
								<p>{{ cmsValue('surgery', 'content', 'tagline_text', 'Let our experienced caregivers help your family') }}</p>
								<div class="two-column">
									<div class="row clearfix">
										<div class="column col-lg-4 col-md-4 col-sm-12">
											<div class="image">
												<img :src="cmsValue('surgery', 'content', 'secondary_image_url', '/frontend/images/resource/service-9.jpg')" alt="" />
											</div>
										</div>
										<div class="column col-lg-8 col-md-8 col-sm-12">
											<h4>{{ cmsValue('surgery', 'content', 'secondary_headline', '24/7 Day Care Services Support You to our Clients') }}</h4>
											<p>{{ cmsValue('surgery', 'content', 'secondary_text', 'Many seniors prefer to age in their own home, but this can grow more and more difficult as the years pass. Your loved one doesn\'t need to give up on the familiarity and comfort of their home, simply because they cannot remain completely independent. Our personal care services are specially designed to provide additional assistance to enhance your loved one\'s independence.') }}</p>
										</div>
									</div>
								</div>
								<h3>{{ cmsValue('surgery', 'content', 'services_headline', 'Our personal caregivers offer assistance with activities such as:') }}</h3>
								<ul class="list-style-three">
									<li>{{ cmsValue('surgery', 'content', 'services_item_1', 'Personal grooming, bathing, toileting, and hygiene tasks') }}</li>
									<li>{{ cmsValue('surgery', 'content', 'services_item_2', 'Mobility and transfers') }}</li>
									<li>{{ cmsValue('surgery', 'content', 'services_item_3', 'Medication reminders and monitoring') }}</li>
									<li>{{ cmsValue('surgery', 'content', 'services_item_4', 'Companionship') }}</li>
									<li>{{ cmsValue('surgery', 'content', 'services_item_5', 'Light housekeeping, meal planning, and meal preparation') }}</li>
									<li>{{ cmsValue('surgery', 'content', 'services_item_6', 'Shopping and errands') }}</li>
								</ul>
								<h3>{{ cmsValue('surgery', 'content', 'all_needs_headline', '24/7 Day Support for All Needs') }}</h3>
								<p>{{ cmsValue('surgery', 'content', 'closing_text', 'Our caregivers are ready to provide quality services to everyone in need of assistance, including elders, those who are living with a disability, and those who are in recovery from an injury or living with a chronic condition. We focus on your individual needs, regardless of the reason you need some extra help. Let us help your day-to-day life be simple, comfortable, and safe.') }}</p>
								<div class="service-contact-box">
                  {{ cmsValue('surgery', 'content', 'service_cta_text', 'Let our experienced caregivers help your family today.') }}
                  <br>
                  <router-link to="/contact">{{ cmsValue('surgery', 'content', 'service_cta_link_text', 'Contact our team') }}</router-link>
                  {{ cmsValue('surgery', 'content', 'service_cta_suffix', 'to learn more!') }}
                </div>
							</div>
						</div>
					</div>
					...
					<aside class="sidebar padding-left">
                        ...
                        <div class="sidebar-widget list-widget">
							...
							<ul class="services-list">
								...
								<li><router-link to="/elderlyservice">Elderly Care Service</router-link></li>
								<li><router-link to="/respitecare">Respite Care</router-link></li>
								<li><router-link to="/nursing">Skilled Nursing</router-link></li>
								<li><router-link to="/surgery">24/7 Day Support</router-link></li>
								<li><router-link to="/discharge">Hospital Discharge</router-link></li>
								<li><router-link to="/care">Companion Care</router-link></li>
								<li><router-link to="/chronical">Chronical Condition Care</router-link></li>
								<li class="active"><router-link to="/support">After Surgery Care</router-link></li>
								<li><router-link to="/lifecare">End of Life Care</router-link></li>
								<li><router-link to="/specialcare">Special Needs Care</router-link></li>
							</ul>
						</div>
                        ...
                        <div class="sidebar-widget contact-widget">
							<div class="widget-content" style="background-image: url(/frontend/images/resource/contact-2.jpg);">
								<h3>{{ cmsValue('surgery', 'sidebar', 'headline', 'Find Care Today') }}</h3>
								<router-link to="/contact" class="theme-btn contact-btn">{{ cmsValue('surgery', 'sidebar', 'button_text', 'contact us') }}</router-link>
							</div>
						</div>
                            ...
                            <div class="row clearfix">
                                ...
                                <div class="footer-column col-lg-7 col-md-6 col-sm-12">
                                    <div class="footer-widget logo-widget">
                                        ...
                                        <div class="text">Supporting Your Indepence.</div>
                            ...
                            <div class="row clearfix">
                                ...
                                <div class="footer-column col-lg-5 col-md-6 col-sm-12">
                                    <div class="footer-widget contact-widget">
    									<h2>{{ footerValue('contact_heading', 'Contact Info') }}</h2>
    									<div class="widget-content">
    										<a :href="footerPhoneHref()" class="contact-number">{{ footerValue('contact_phone', '024 7623 1188') }}</a>
    										<ul>
    											<li v-html="footerValue('contact_address', 'Office GE13, 101 Lockhurst Lane,<br> Coventry, CV6 5sf')"></li>
    											<li>{{ footerValue('contact_email_label', 'Email') }} :<a :href="footerMailtoHref()"> {{ footerValue('contact_email', 'info@facilitatecareservices.co.uk') }}</a></li>
    										</ul>
    									</div>
    								</div>
    							</div>
