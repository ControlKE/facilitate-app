...
function fetchComplaintRows(string $source): array
{
    ...
    if (!$hasSuccessfulQuery) {
        throw new RuntimeException('Failed to load complaints from available data sources.');
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $action === 'getComplaint') {
    try {
        $source = normalizeDbSource((string) ($_GET['source'] ?? 'auto'));
        $data = fetchComplaintRows($source);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'deleteComplaint') {
