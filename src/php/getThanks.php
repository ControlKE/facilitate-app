...
function fetchThanksRows(string $source): array
{
    ...
    foreach ($connections as $connectionSource => $conn) {
        $result = $conn->query('SELECT * FROM carerthanks');
        if ($result !== false) {
    ...
    if (!$hasSuccessfulQuery) {
        throw new RuntimeException('Failed to load thank-you messages from available data sources.');
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $action === 'getThanks') {
    try {
        $source = normalizeDbSource((string) ($_GET['source'] ?? 'auto'));
        $data = fetchThanksRows($source);
        echo json_encode($data);
    } catch (Throwable $exception) {
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Failed to load thank-you messages.']);
    }
    exit;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'deleteThanks') {
    ...
    $stmt = $conn->prepare('DELETE FROM carerthanks WHERE ID = ?');
    if ($stmt === false) {
        http_response_code(500);
